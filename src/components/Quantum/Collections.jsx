...
const Collections = () => {
  const { data, isLoading, error } = useQuery('quantums', fetchQuantums, { staleTime: 1000 * 60 * 5 })
  const purchasedQuantum = data?.filter((item) => `${item.$id}` === localStorage.getItem(`quantum${item.$id}`))
  ...
  if (error) {
    return <div className="text-red-500 text-center p-4 bg-red-100 rounded-lg">{(error ).message}</div>
  }
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
            {purchasedQuantum.map((item, ind) => (
              ...
              >
                ...
                <div className="p-6">
                  <h3 className="text-xl font-semibold mb-2 text-capitalize text-gray-800 truncate">{item.title}</h3>
                  <span className="text-sm text-indigo-600 bg-indigo-100 px-2 py-1 rounded-full">{item.code}</span>
                  <div className="mt-4 flex justify-between items-center">
                    <Link to={`/collections/read?q=${item.url}`} className="flex items-center text-sm text-purple-600 hover:text-purple-800 transition-colors duration-300">
                      <BookOpen className="w-4 h-4 mr-1" />
                      Read
                    </Link>
                    <button className="flex items-center text-sm text-indigo-600 hover:text-indigo-800 transition-colors duration-300">
